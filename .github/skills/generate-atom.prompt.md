...
Use `#prompt:tpl-component` with `type` = `a` and the component's inputs to generate this file. If `properties` are provided, pass them to `#prompt:tpl-component` so it renders property-aware JSX.
...
import { ButtonTemplate } from '@templates/button/Button';

const ButtonPage = () => {
  return <ButtonTemplate buttonData={buttonData} />;
};
...
export const defaultButtonPage: Story = {
  name: 'Button - Default',
  path: 'default',
  render: () => {
    return <ButtonTemplate buttonData={buttonData} />;
...
Use `#prompt:tpl-wrapper` with `componentTypePlural` = `atoms` to generate this file.

**Expected output** (example for `Button`):
```tsx
...
- The type file uses **append** mode — add content to the end of `src/_types/atoms.d.ts`, don't overwrite it
