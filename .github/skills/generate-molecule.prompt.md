---
description: "Generate a Molecule component with optional style, state, script, type definition, page, template, and data file"
---
...
**Path**: `src/molecules/{kebab-name}/{componentName}.tsx`

Use `#prompt:tpl-component` with `type` = `m` and the component's inputs to generate this file. If `properties` are provided, pass them to `#prompt:tpl-component` so it renders property-aware JSX.
...
**Path**: `src/molecules/{kebab-name}/{componentName}.scss`

Use `#prompt:tpl-style` with `type` = `m` to generate this file.

### 4. State file (optional)

**Path**: `src/molecules/{kebab-name}/{componentName}.states.json`
...
```tsx
import { searchBarData } from '@data/search-bar';
import { SearchBarTemplate } from '@templates/search-bar/SearchBar';

const SearchBarPage = () => {
  return <SearchBarTemplate searchBarData={searchBarData} />;
};
...
import { SearchBarTemplate } from '@templates/search-bar/SearchBarTemplate';
import { searchBarData } from '@data/search-bar';
...
export const defaultSearchBarPage: Story = {
...
Use `#prompt:tpl-wrapper` with `componentTypePlural` = `molecules` to generate this file.

**Expected output** (example for `SearchBar`):
```tsx
import { SearchBarModel } from '@_types/types';
...
export const SearchBarTemplate = (model: Props) => {
  const { searchBarData } = model;
...
- The type file uses **append** mode — add content to the end of `src/_types/molecules.d.ts`, don't overwrite it
...
- When generating the template component, replace `@organisms/` with `@molecules/` in the import path
