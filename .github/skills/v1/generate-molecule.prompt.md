...
# Generate Molecule Component (v1)
...
Use `tpl-component` with `type` = `m` and the component's inputs to generate this file. If `properties` are provided, pass them to `tpl-component` so it renders the destructuring line.
...
Use `tpl-style` with `type` = `m` to generate this file.

### 4. State file (optional)

**Path**: `src/molecules/{kebab-name}/{componentName}.states.json`

Use `tpl-state` with `type` = `m` to generate this file.
...
Use `tpl-template` with `componentTypePlural` = `molecules` to generate this file. See `tpl-template` for expected output.
