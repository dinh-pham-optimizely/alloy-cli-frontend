...
Read `.alloy-models.json` from the project root. This is a compact JSON file generated by `alloy-cli-frontend scan` that lists all known model names by category:
...
If `.alloy-models.json` does not exist, skip project type matching entirely and use only LLM inference (Step 2, Priority 3). Suggest the user run `alloy-cli-frontend scan` to generate the registry.
...
- **`#prompt:tpl-type`** — properties become optional fields in the interface
- **`#prompt:tpl-component`** — properties appear ONLY in the destructuring line (`const { ... } = model;`)
